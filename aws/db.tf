...
resource "aws_db_instance" "rds" {
  ...
  multi_az               = false
  db_name                = "mage"
  username               = "${jsondecode(data.aws_secretsmanager_secret_version.db_credentials_latest.secret_string)["user"]}"
  password               = "${jsondecode(data.aws_secretsmanager_secret_version.db_credentials_latest.secret_string)["password"]}"
